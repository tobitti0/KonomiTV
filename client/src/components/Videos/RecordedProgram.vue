<template>
    ...
    <OfflineVideoDownloadDialog :program="displayProgram" v-model:show="showOfflineDownload" />
...
// 保存済みオフライン動画はサーバー側の Box ファイルが取得不能でも再生できる
// それ以外は、保存ジョブの操作中・失敗中、または再生元を取得できない場合に誤遷移しないよう div へ切り替える
const rootTag = computed(() => {
    if (props.forOffline === true && props.offlineVideo !== null) return 'router-link';
    if (isOfflineInteractionBlocked.value === true || isPlayable.value === false) return 'div';
    return 'router-link';
});
...
const rootBindings = computed(() => {
    ...
    if (props.forOffline === true && props.offlineVideo !== null) {
        return { to: `/videos/watch/${props.program.id}?source=offline` };
    }
    return {
        to: isPlayable.value
            ? `/videos/watch/${props.program.id}`
            : { path: '' },
    };
});
